...
function DashboardHome({
  ...
  return (
    <section className="dashboard" aria-labelledby="dashboard-title" aria-busy={isRefreshing}>
      ...
      <header className="dashboard__hero"><div className="dashboard__hero-heading"><span className="dashboard__hero-icon" aria-hidden="true"><Gauge size={25} strokeWidth={2.1} /></span><div><h1 id="dashboard-title">システムダッシュボード</h1></div></div><div className="dashboard__hero-meta"><div className="dashboard__hero-badges"><span className={`dashboard__api-status dashboard__api-status--${summaryError ? 'error' : isRefreshing ? 'loading' : 'connected'}`}><span className="dashboard__api-dot" aria-hidden="true" /><span>{summaryError ? 'API エラー' : isRefreshing ? 'API 読み込み中' : 'API 接続中'}</span></span></div><div className="dashboard__update-row"><span className="dashboard__updated-label">最終更新</span><time className="dashboard__updated-time" dateTime={lastUpdated ?? undefined}>{formatDateTime(lastUpdated)}</time><button className="dashboard__refresh-button" type="button" aria-label={isRefreshing ? 'Dashboardを更新中' : 'Dashboardを更新'} disabled={isRefreshing} onClick={onRefresh}><RefreshCw className={isRefreshing ? 'dashboard__refresh-icon is-spinning' : 'dashboard__refresh-icon'} size={16} aria-hidden="true" /></button></div></div></header>

      {summaryError && <div className="dashboard__error dashboard__error--inline" role="alert"><CircleAlert size={17} aria-hidden="true" /><p>{summaryError}</p><button className="dashboard__retry-button" type="button" disabled={isRefreshing} onClick={onRefresh}>再試行</button></div>}
      <section className="dashboard__section" aria-labelledby="dashboard-overview-title"><div className="dashboard__section-heading"><div><h2 id="dashboard-overview-title">システム概要</h2></div><span className="dashboard__section-count">7項目</span></div><div className="dashboard__kpi-grid dashboard__summary-grid">
