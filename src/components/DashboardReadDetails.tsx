...
export function DataStoreDetails({ apiRevision, refreshRevision = 0 }: { apiRevision: number; refreshRevision?: number }) {
  ...
  if (!data) return <DetailFrame title="DataStore" query={query}>{null}</DetailFrame>

  const connection = data.isConnected === true
    ...
    <DetailFrame title="DataStore" query={query}>
      <DetailSection title="接続状態">
        <div className="dashboard-detail__metrics">
          <MetricCard label="接続" value={<StatusBadge tone={connection.tone} icon={connection.icon}>{connection.label}</StatusBadge>} icon={Database} tone={connection.tone} />
          <MetricCard label="Provider version" value={data.providerVersion || '—'} icon={Server} tone="info" />
          <MetricCard label="Store" value={data.storeName || statistics?.storeName || '—'} icon={HardDrive} tone="muted" />
          <MetricCard label="Availability" value={<StatusBadge tone={availabilityStatus.tone} icon={availabilityStatus.icon}>{availabilityStatus.label}</StatusBadge>} icon={Database} tone={availabilityStatus.tone} />
        </div>
        <dl className="dashboard-detail__definition-grid">
          <div><dt>Diagnostic code</dt><dd>{data.diagnosticCode || data.topology?.diagnosticCode || '—'}</dd></div>
          <div><dt>Topology</dt><dd>{data.topology?.state || '—'}</dd></div>
          <div><dt>Available stores</dt><dd>{statistics?.availableStoreNames?.join(', ') || '—'}</dd></div>
        </dl>
      ...
      <DetailSection title="統計">
        <div className="dashboard-detail__metrics">
          <MetricCard label="Books" value={formatNumber(statistics?.bookCount)} icon={Database} tone="info" />
          <MetricCard label="Tags" value={formatNumber(statistics?.tagCount)} icon={Database} tone="muted" />
          <MetricCard label="Indexes" value={formatNumber(statistics?.indexCount)} icon={Server} tone="muted" />
          <MetricCard label="Data size" value={formatBytes(statistics?.dataSizeBytes)} icon={HardDrive} tone="muted" />
        </div>
        <div className="dashboard-detail__meta"><span>Observed {formatDateTime(statistics?.observedAt)}</span></div>
      </DetailSection>
      <DetailSection title="Resources" count={formatNumber(resources.length)}>
        <div className="dashboard-detail__table-wrap" role="region" aria-label="DataStoreリソース" tabIndex={0}>
          <table className="dashboard-detail__table">
            <thead><tr><th scope="col">Name</th><th scope="col">Records</th><th scope="col">Size</th><th scope="col">Indexes</th><th scope="col">Availability</th></tr></thead>
            <tbody>{resources.map((resource, index) => {
              const status = getDashboardStatus(resource.availability)
              return <tr key={`${resource.name ?? 'resource'}-${index}`}><th scope="row">{resource.name || '—'}</th><td>{formatNumber(resource.recordCount)}</td><td>{formatBytes(resource.sizeBytes)}</td><td>{formatNumber(resource.indexCount)}</td><td><StatusBadge tone={status.tone} icon={status.icon}>{status.label}</StatusBadge></td></tr>
            })}</tbody>
          </table>
          {!resources.length && <div className="dashboard-detail__empty">—</div>}
        </div>
        {hasIndexes && <div className="dashboard-detail__table-wrap" role="region" aria-label="DataStoreインデックス" tabIndex={0}>
          <table className="dashboard-detail__table">
            <thead><tr><th scope="col">Resource</th><th scope="col">Index</th><th scope="col">Definition</th><th scope="col">Unique</th><th scope="col">Availability</th></tr></thead>
            <tbody>{resources.flatMap((resource, resourceIndex) => (resource.indexes ?? []).map((indexInfo, indexIndex) => {
              const status = getDashboardStatus(indexInfo.availability)
              return <tr key={`${resource.name ?? resourceIndex}-${indexInfo.name ?? indexIndex}`}><th scope="row">{resource.name || '—'}</th><td>{indexInfo.name || '—'}</td><td><code className="dashboard-detail__value-truncate">{indexInfo.definition || '—'}</code></td><td>{indexInfo.isUnique ? 'Yes' : 'No'}</td><td><StatusBadge tone={status.tone} icon={status.icon}>{status.label}</StatusBadge></td></tr>
            }))}</tbody>
          </table>
        </div>}
      </DetailSection>
...
export function DataFolderDetails({ apiRevision, refreshRevision = 0 }: { apiRevision: number; refreshRevision?: number }) {
  ...
  const tone = percent !== null && percent >= 90 ? 'danger' as const : percent !== null && percent >= 75 ? 'warning' as const : 'success' as const
...
export function LogsDetails({ apiRevision, refreshRevision = 0 }: { apiRevision: number; refreshRevision?: number }) {
  ...
  const clearLogs = async () => {
    ...
    try {
      ...
      if (response.success === false) throw new ApiError(response.message ?? 'ログのクリアに失敗しました。', { category: 'server' })
