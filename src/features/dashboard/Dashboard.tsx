...
import { getDashboardRouteTitle, resolveDashboardRoute } from './dashboard-routes'
import { Button } from '../../components/ui'
import './dashboard.css'

export type { DashboardRoute } from './dashboard-routes'
// Keep the legacy helper import path stable for route-aware callers.
// oxlint-disable-next-line react/only-export-components
export { resolveDashboardRoute } from './dashboard-routes'
...
const getDataStoreStatus = (summary?: DataStoreSummary | null): StatusPresentation => {
  ...
  if (summary.isConnected === true)
    return { label: '接続中', tone: 'success', icon: STATUS_ICONS.success }
  if (summary.isConnected === false)
    return { label: '切断', tone: 'danger', icon: STATUS_ICONS.danger }
...
const getAvailabilityStatus = (summary?: DataFolderSummary | null): StatusPresentation => {
  if (!summary || summary.isAvailable === undefined || summary.isAvailable === null)
...
const getServiceStatus = (
  summary?: WebPilotSummary | ImageWorkerSummary | null,
): StatusPresentation => {
  if (!summary) return UNKNOWN_STATUS
  ...
  if (summary.isConfigured === false)
    return { label: '未設定', tone: 'warning', icon: STATUS_ICONS.warning }
  return UNKNOWN_STATUS
}
...
const getMaintenanceStatus = (summary?: MaintenanceSummary | null): StatusPresentation => {
  ...
  if (typeof summary.issuesCount === 'number')
    return summary.issuesCount > 0
...
const getCacheStatus = (summary?: CacheSummary | null): StatusPresentation => {
  if (!summary) return UNKNOWN_STATUS
  if (typeof summary.hitRate === 'number')
...
const usageRatio = (summary?: DataFolderSummary | null) => {
  if (
    !summary ||
    typeof summary.freeBytes !== 'number' ||
...
const sectionError = (errors: Record<string, string> | null | undefined, names: string[]) => {
  ...
  const match = Object.entries(errors).find(
    ([key, value]) => wanted.includes(key.toLocaleLowerCase('en-US')) && value,
  )
  return match?.[1] ?? null
}
...
const StatusBadge = ({
  ...
  return (
    <span
      className={`dashboard__status dashboard__status--${status.tone} ${compact ? 'dashboard__status--compact' : ''}`}
    >
...
function SummaryCard({
  ...
  return (
    <InternalLink
      className="dashboard__card dashboard__summary-card dashboard__link-card"
      href={href}
    >
      <div className="dashboard__card-heading">
        ...
        <h3>{title}</h3>
        <StatusBadge status={displayedStatus} compact />
      </div>
      <div className="dashboard__summary-card-body">
        {loading ? (
          <>
            <span className="dashboard__summary-value-skeleton" aria-hidden="true" />
            <span className="dashboard__summary-loading-label" role="status">
              取得中
            </span>
          </>
        ) : (
      ...
      {error && (
        <p className="dashboard__card-error">
...
function ActivityEntry({ entry }: { entry: DashboardLogEntryDto }) {
  ...
  return (
    <li className="dashboard__activity-entry">
      <span className="dashboard__activity-level">
...
function DashboardHome({
  ...
  return (
    <section className="dashboard" aria-labelledby="dashboard-title" aria-busy={isRefreshing}>
      <p className="dashboard__live-region sr-only" role="status" aria-live="polite">
        {announcement}
      </p>
      <header className="dashboard__hero">
        <div className="dashboard__hero-heading">
          <span className="dashboard__hero-icon" aria-hidden="true">
        ...
        <div className="dashboard__hero-meta">
          <div className="dashboard__hero-badges">
            <span
              className={`dashboard__api-status dashboard__api-status--${summaryError ? 'error' : isRefreshing ? 'loading' : 'connected'}`}
            >
              <span className="dashboard__api-dot" aria-hidden="true" />
              <span>
                {summaryError ? 'API エラー' : isRefreshing ? 'API 読み込み中' : 'API 接続中'}
              </span>
            </span>
          </div>
          <div className="dashboard__update-row">
            <span className="dashboard__updated-label">最終更新</span>
            <time className="dashboard__updated-time" dateTime={lastUpdated ?? undefined}>
            ...
            >
              <RefreshCw
                className={
                  isRefreshing ? 'dashboard__refresh-icon is-spinning' : 'dashboard__refresh-icon'
                }
                size={16}
                aria-hidden="true"
              />
      ...
      <section className="dashboard__section" aria-labelledby="dashboard-overview-title">
        <div className="dashboard__section-heading">
          <div>
            <h2 id="dashboard-overview-title">システム概要</h2>
          </div>
          <span className="dashboard__section-count">8項目</span>
        </div>
        <div className="dashboard__kpi-grid dashboard__summary-grid">
          <SummaryCard
            loading={isInitialLoading}
            href="/dashboard/datastore"
            title="DataStore"
            icon={Database}
            status={getDataStoreStatus(summary?.dataStore)}
            error={sectionError(errors, ['dataStore', 'datastore', 'mongoDb', 'mongodb'])}
          >
            <strong>{summary?.dataStore?.providerVersion ?? '—'}</strong>
            <span>{summary?.dataStore?.storeName ?? '接続情報未取得'}</span>
          </SummaryCard>
          <SummaryCard
            loading={isInitialLoading}
            href="/dashboard/datafolder"
            title="DataFolder"
            icon={FolderOpen}
            status={getAvailabilityStatus(summary?.dataFolder)}
            error={sectionError(errors, ['dataFolder', 'datafolder'])}
          >
            <strong>{dataFolderPercent === null ? '—' : `${dataFolderPercent}%`}</strong>
            <span>
              空き {formatBytes(summary?.dataFolder?.freeBytes)} /{' '}
              {formatBytes(summary?.dataFolder?.totalBytes)}
            </span>
          </SummaryCard>
          <SummaryCard
            loading={isInitialLoading}
            href="/dashboard/webpilot"
            title="WebPilot"
            icon={Server}
          ...
          <SummaryCard
            ...
            title="Maintenance"
            icon={Wrench}
            status={getMaintenanceStatus(summary?.maintenance)}
            error={sectionError(errors, ['maintenance'])}
          >
            <strong>{formatCount(summary?.maintenance?.issuesCount)}件</strong>
            <span>最終 {formatDateTime(summary?.maintenance?.lastExecutionTime)}</span>
          </SummaryCard>
          <SummaryCard
            loading={isInitialLoading}
            ...
            title="Cache"
            icon={Zap}
            status={getCacheStatus(summary?.cache)}
            error={sectionError(errors, ['cache'])}
          >
            <strong>{cachePercent === null ? '—' : `${cachePercent}%`}</strong>
            <span>
              {formatCount(summary?.cache?.entryCount)}エントリ ·{' '}
              {formatBytes(summary?.cache?.sizeBytes)}
            </span>
          </SummaryCard>
          <SummaryCard
            href="/dashboard/web-cache"
            title="Web Cache"
            icon={Server}
          ...
          <SummaryCard
            ...
            title="Logs"
            icon={ListChecks}
            status={summary === null ? UNKNOWN_STATUS : logStatus}
            error={logsError ?? sectionError(errors, ['logs'])}
          >
            <strong>{formatCount(errorCount)}件</strong>
            <span>過去24時間の ERROR</span>
          </SummaryCard>
        </div>
      </section>
          ...
          {latestLogs.length > 0 ? (
            <ul className="dashboard__activity-list">
              {latestLogs.map((entry, index) => (
                <ActivityEntry
                  entry={entry}
                  key={entry.sequence ?? `${entry.timestamp}-${entry.message}-${index}`}
                />
              ))}
            </ul>
          ) : (
            <p className="dashboard__empty">ログはありません。</p>
          )}
        </article>
      </section>
    </section>
...
export function Dashboard({ path, apiRevision }: DashboardProps) {
  ...
  const refreshSummary = useCallback(
    (reset = false, parentSignal?: AbortSignal) => {
      ...
      setAnnouncement(
        hasData ? 'Dashboard概要を更新しています。' : 'Dashboard概要を読み込んでいます。',
      )
      return getDashboardSummary(controller.signal)
        .then((response) => {
          if (controller.signal.aborted) return
          if (response.success === false)
            throw new Error(response.message ?? 'Dashboard概要を取得できませんでした。')
  ...
  const refreshLogs = useCallback(
    (reset = false, parentSignal?: AbortSignal) => {
      ...
      if (reset) {
        logsRef.current = EMPTY_LOGS
        setLogs(EMPTY_LOGS)
        setLogsError(null)
      }
      return getDashboardLogEntries(0, 'Warning', controller.signal)
        .then((response) => {
          if (controller.signal.aborted) return
          if (response.success === false)
            throw new Error(response.message ?? 'ログを取得できませんでした。')
  ...
  const refreshHome = useCallback(
    (reset = false, parentSignal?: AbortSignal) =>
      Promise.all([refreshSummary(reset, parentSignal), refreshLogs(reset, parentSignal)]).then(
        () => undefined,
      ),
  ...
  return (
    <DashboardHome
      ...
      isInitialLoading={summary === null && summaryStatus === 'loading'}
      announcement={announcement || 'Dashboardを読み込んでいます。'}
      summaryError={summaryError}
      logsError={logsError}
      onRefresh={() => refreshHome(summary === null)}
    />
  )
}
