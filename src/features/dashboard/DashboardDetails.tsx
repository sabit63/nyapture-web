...
import { CacheDetails, MaintenanceDetails, ServiceDetails } from './DashboardActionDetails'
import { DataFolderDetails, LogsDetails, DataStoreDetails } from './DashboardReadDetails'
import { InternalLink } from '../../app/client-router'
import { buttonClassName, IconButton } from '../../components/ui'
import './dashboard-details.css'
import { WebCacheManagement } from '../web-cache/WebCacheManagement'
import { DASHBOARD_ROUTE_META, type DashboardDetailRoute } from './dashboard-routes'

export type { DashboardDetailRoute } from './dashboard-routes'
// Keep the legacy helper import path stable for route-aware callers.
// oxlint-disable-next-line react/only-export-components
export { getDashboardRouteTitle } from './dashboard-routes'
  ...
  return (
    <section className="dashboard-detail" aria-labelledby="dashboard-detail-title">
      <header className="dashboard-detail__header">
        <InternalLink
          className={buttonClassName(
            { variant: 'ghost', tone: 'neutral', size: 'compact' },
            'dashboard-detail__icon-button',
          )}
          href="/dashboard"
      ...
      <div className="dashboard-detail__body">
        {(route === 'datastore' || route === 'mongodb') && (
          <DataStoreDetails apiRevision={apiRevision} refreshRevision={refreshRevision} />
        )}
        {route === 'datafolder' && (
          <DataFolderDetails apiRevision={apiRevision} refreshRevision={refreshRevision} />
        )}
        {route === 'webpilot' && (
          <ServiceDetails
            service="webpilot"
        ...
        {route === 'maintenance' && (
          <MaintenanceDetails apiRevision={apiRevision} refreshRevision={refreshRevision} />
        )}
        {route === 'cache' && (
