...
export async function getBookRecommendations(groupId: string, bookId: string, targetType: RecommendationType, signal: AbortSignal, limit = 20): Promise<RecommendationResult> {
  return getRecommendations(`/api/book/${encodeURIComponent(groupId)}/${encodeURIComponent(bookId)}/recommendations`, { targetType, limit }, signal)
}

export async function getEntityRecommendations(entityType: 'Artist' | 'Group', tagName: string, targetType: 'Artist' | 'Group', signal: AbortSignal, limit = 20): Promise<RecommendationResult> {
  return getRecommendations(`/api/recommendations/entities/${entityType}`, { tagName, targetType, limit }, signal)
}
...
export function usableRecommendations(items: RecommendationHit[] | null | undefined, type: RecommendationType, limit = 20) {
