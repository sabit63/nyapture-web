...
it('keeps disabled status actions visible and replaces them with selection in selection mode', async () => {
  ...
  try {
    ...
    assert.equal(container.querySelector('.card-actions'), null)
    await act(async () => { root.render(createElement(BookCard, { ...props, selectMode: true, selected: true })) })
    assert.ok(container.querySelector('.book-card--selected'))
    // Failed bulk items retain their selection keys after selection mode closes.
    await act(async () => { root.render(createElement(BookCard, { ...props, selectMode: false, selected: true })) })
    assert.equal(container.querySelector('.book-card--selected'), null)
    assert.equal(container.querySelector('.card-select'), null)
    assert.ok(container.querySelector('.card-actions'))
    await act(async () => { root.render(createElement(BookCard, { ...props, actionFailed: true })) })
    assert.ok(container.querySelector('.book-card--action-failed'))
    await act(async () => { root.render(createElement(BookCard, { ...props, actionFailed: true, selectMode: true, selected: true })) })
    assert.ok(container.querySelector('.book-card--selected'))
    assert.equal(container.querySelector('.book-card--action-failed'), null)
    await act(async () => { root.render(createElement(BookCard, { ...props, actionFailed: false })) })
    assert.equal(container.querySelector('.book-card--action-failed'), null)
  } finally {
    await act(async () => { root.unmount() })
    container.remove()
