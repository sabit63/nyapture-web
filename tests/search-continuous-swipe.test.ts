...
it('changes books for deliberate touch, mouse, and trackpad gestures outside controls', async () => {
  ...
  try {
    ...
    assert.equal(container.querySelector('.continuous-reader__navigator-body'), null)
    await act(async () => { (progressButton as HTMLButtonElement).click() })
    ...
    assert.equal(navigatorRows[0].querySelector('strong')?.textContent, 'First')
    assert.equal(navigatorRows[0].querySelector('.continuous-reader__navigator-meta')?.textContent, '1ページ閲覧中')
    assert.equal(navigatorRows[0].getAttribute('aria-current'), 'true')
    assert.equal(navigatorRows[1].querySelector('.continuous-reader__navigator-current'), null)
    assert.equal(navigatorRows[0].querySelector('.continuous-reader__navigator-thumbnail button'), null)
    await act(async () => { navigatorRows[1].click() })
    assert.equal(jumps.pop(), books[1])
    assert.equal(container.querySelector('.continuous-reader__navigator-body'), null)
...
it('opens recommendations for the active continuous book, isolates gestures, and resets on book change', async () => {
  ...
  try {
    ...
    assert.equal(new URLSearchParams(dom.window.location.search).get('tag'), 'Artists:artist-name')
